...
unsigned int af_vcm_vendor;

struct imx135_info {
	struct miscdevice		miscdev_info;
	int				mode;
	struct imx135_power_rail	power;
	struct imx135_otp otp_data;
...
#define IMX135_USE_DIGITAL_GAIN
...
/* standard */
static struct imx135_reg mode_4208x3120_n[] = {
	/* global settings */
	{0x0101, 0x00},
	{0x0105, 0x01},
	{0x0110, 0x00},
...
/* low frame rate */
static struct imx135_reg mode_4208x3120_l[] = {
/* global settings */
	{0x0101, 0x00},
	{0x0105, 0x01},
	{0x0110, 0x00},
...
static struct imx135_reg mode_2104x1560_n[] = {
/* global settings */
	{0x0101, 0x00},
	{0x0105, 0x01},
	{0x0110, 0x00},
...
static struct imx135_reg mode_1920x1080_n[] = {
	/* global settings */
	{0x0101, 0x00},
	{0x0105, 0x01},
	{0x0110, 0x00},
...
enum {
	...
	IMX135_MODE_4208X3120_NHDR,
	IMX135_MODE_4208X3120_LCLK,
	IMX135_MODE_1920X1080,
	IMX135_MODE_1920X1080_NHDR,
	IMX135_MODE_1280X720,
	IMX135_MODE_2616X1472,
	IMX135_MODE_2104X1560,
	IMX135_MODE_2104X1560_NHDR,
...
static struct imx135_reg *mode_table[] = {
	...
	[IMX135_MODE_4208X3120_NHDR] = mode_4208x3120_n,
	[IMX135_MODE_4208X3120_LCLK] = mode_4208x3120_l,
	[IMX135_MODE_1920X1080] = mode_1920x1080,
	[IMX135_MODE_1920X1080_NHDR] = mode_1920x1080_n,
	[IMX135_MODE_1280X720]  = mode_1280x720,
	[IMX135_MODE_2616X1472]  = mode_2616x1472,
	[IMX135_MODE_2104X1560]  = mode_2104x1560,
	[IMX135_MODE_2104X1560_NHDR] = mode_2104x1560_n,
...
#ifdef IMX135_USE_DIGITAL_GAIN
static inline void
imx135_get_digital_gain_reg(struct imx135_reg *regs, u16 gain)
{
	regs->addr = 0x020E;
	...
	(regs + 2)->addr = 0x0210;
	(regs + 2)->val = (gain >> 8) & 0xff;
	(regs + 3)->addr = 0x0211;
	(regs + 3)->val = (gain) & 0xff;

	(regs + 4)->addr = 0x0212;
	(regs + 4)->val = (gain >> 8) & 0xff;
	(regs + 5)->addr = 0x0213;
	(regs + 5)->val = (gain) & 0xff;

	(regs + 6)->addr = 0x0214;
	(regs + 6)->val = (gain >> 8) & 0xff;
	(regs + 7)->addr = 0x0215;
	(regs + 7)->val = (gain) & 0xff;
}
#endif
...
imx135_set_mode(struct imx135_info *info, struct imx135_mode *mode)
{
	...
	pr_info
		("%s: xres %u yres %u framelength %u coarsetime %u gain %u, hdr_en:%d nsl_en:%d\n",
		 __func__, mode->xres, mode->yres, mode->frame_length,
		mode->coarse_time, mode->gain, mode->hdr_en, mode->nsl_en);

	if (mode->xres == 4208 && mode->yres == 3120) {
		if (mode->hdr_en)
			sensor_mode = IMX135_MODE_4208X3120;
		else if (mode->nsl_en)
			sensor_mode = IMX135_MODE_4208X3120_NHDR;
		else
			sensor_mode = IMX135_MODE_4208X3120_LCLK;
	} else if (mode->xres == 1920 && mode->yres == 1080) {
		sensor_mode =
			mode->hdr_en ? IMX135_MODE_1920X1080 : IMX135_MODE_1920X1080_NHDR;
	} else if (mode->xres == 1280 && mode->yres == 720) {
		sensor_mode = IMX135_MODE_1280X720;
	} else if (mode->xres == 2104 && mode->yres == 1560) {
		sensor_mode =
			mode->hdr_en ? IMX135_MODE_2104X1560 : IMX135_MODE_2104X1560_NHDR;
	...
	pr_info("imx135 write hdr:%d mode:%d", mode->hdr_en, sensor_mode);
	...
	if (mode->hdr_en) {
		...
		imx135_get_coarse_time_short_regs(reg_list + 6,
...
imx135_set_gain(struct imx135_info *info, u16 gain, bool group_hold)
{
	...
	imx135_write_reg(info->i2c_client, reg_list.addr, gain);
	/* writing second gain register for HDR */
	imx135_write_reg(info->i2c_client, 0x233, gain);

	imx135_get_digital_gain_reg(dreg_list, digital_gain);
	for (i = 0; i < 8; i++) {
		imx135_write_reg(info->i2c_client, dreg_list[i].addr,
				 dreg_list[i].val);
	}
#else
...
static int imx135_wait_otp_page(struct imx135_info *info, u16 page)
{
	int i;
	u8 bak = 0;

	imx135_write_reg(info->i2c_client, 0x3B02, page);
	imx135_write_reg(info->i2c_client, 0x3B00, 0x01);
	udelay(10);
	for (i = 0; i < 10; i++) {
		imx135_read_reg(info->i2c_client, 0x3B01, &bak);
		bak &= 1;
		if (bak == 1)
			break;
		udelay(10);
		pr_info("%s bak:%d", __func__, bak);
	}
	return bak;
}

static int imx135_get_otp_vendor(struct imx135_info *info)
{
	int i;
	u8 bak = 0;
	u8 baj = 0;
	...
	if (imx135_wait_otp_page(info, 1) == 1) {
		for (i = 0x3B24; i >= 0x3B04; i -= 0x10) {
			imx135_read_reg(info->i2c_client, i, &bak);
			imx135_read_reg(info->i2c_client, i + 1, &baj);
			if (bak != 0)
				break;
		}
	}
	pr_debug("imx135 module vendor: 0x%02x %02x", bak, baj);
	if ((bak == 0x1D) && (baj == 0x02))
		otp[0] = 1;
...
static int imx135_get_otp_data(struct imx135_info *info)
{
	int ret = 0;
	int i, j;
	u8 bak = 0;
	...
	pr_info("imx135 vendor: 0x%02x %02x", bak, baj);
	af_vcm_vendor = bak & 0xF0;

	if (imx135_wait_otp_page(info, 15) == 1) {
		for (i = 0x3B14; i >= 0x3B04; i -= 0x10) {
			imx135_read_reg(info->i2c_client, i, &bak);
	...
	pr_info("imx135 otp: 0x%02x", bak);
	if (bak == 0xEE)
		return ret;

	for (j = 2; j < 14; j++) {
		imx135_wait_otp_page(info, j);
		for (i = 0; i < 64; i++) {
			ret |=
				imx135_read_reg(info->i2c_client, 0x3B04 + i, otp);
			otp++;
		}
	}
	for (i = 0; i < 35; i++) {
		imx135_wait_otp_page(info, j);
		ret |= imx135_read_reg(info->i2c_client, 0x3B04 + i, otp);
		otp++;
	}
	pr_info("%s size:%d", __func__, otp - info->otp_data.otp_data);
	info->otp_data.otp_size = 803;
	return ret;
}

static long imx135_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int err = 0;
	...
	case IMX135_IOCTL_GET_OTPDATA:
		{
			err = imx135_get_otp_data(info);
			...
			if (copy_to_user((void __user *)arg, &info->otp_data,
					 sizeof(struct imx135_otp))) {
				pr_info
					("%s:Failed to copy otp data to user space\n",
					 __func__);
				return -EFAULT;
			}
			return 0;
		}
	case IMX135_IOCTL_GET_OTPVEND:
	{
			err = imx135_get_otp_vendor(info);
			if (copy_to_user((void __user *)arg, &info->otp_data,
					sizeof(struct imx135_otp))) {
